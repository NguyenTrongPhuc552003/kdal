...
static void test_multi_device_loopback(int fd)
{
	...
	for (i = 0; i < 4; i++) {
		...
		CHECK(select_device(fd, devices[i]) == 0, "select %s",
		      devices[i]);
...
static void test_power_management(int fd)
{
	...
	CHECK(info.power_state == 2, "power_state = suspend (%u)",
	      info.power_state);

	/* Resume */
	state = 1; /* KDAL_POWER_ON */
	CHECK(ioctl(fd, KDAL_IOCTL_SET_POWER, &state) == 0, "set power → on");
}
...
static void test_error_paths(int fd)
{
	...
	/* Read without selection (new fd) */
	{
		...
		if (fd2 >= 0) {
			...
			CHECK(n < 0, "read without select fails (errno=%d)",
			      errno);
			close(fd2);
		}
	}
