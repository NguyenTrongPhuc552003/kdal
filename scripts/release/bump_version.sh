...
#
# Usage: ./scripts/release/bump_version.sh <major.minor.patch>
# Example: ./scripts/release/bump_version.sh 0.2.0
...
# 5. VERSION (top-level, read by CMakeLists.txt)
info "Updating VERSION..."
echo "$NEW_VERSION" >VERSION

# 6. VS Code extension package.json
if [ -f editor/vscode/kdal-lang/package.json ]; then
	info "Updating VS Code extension version..."
	sed -i.bak "s/\"version\": \"[^\"]*\"/\"version\": \"$NEW_VERSION\"/" \
		editor/vscode/kdal-lang/package.json
	rm -f editor/vscode/kdal-lang/package.json.bak
fi

# 7. kdalup.sh
if [ -f scripts/installer/kdalup.sh ]; then
	info "Updating kdalup version..."
	sed -i.bak "s/^KDALUP_VERSION=\"[^\"]*\"/KDALUP_VERSION=\"$NEW_VERSION\"/" \
		scripts/installer/kdalup.sh
	rm -f scripts/installer/kdalup.sh.bak
fi

# 8. Homebrew formula URL
if [ -f packaging/homebrew/kdal.rb ]; then
	info "Updating Homebrew formula version..."
	sed -i.bak "s|/archive/refs/tags/v[^\"]*\.tar\.gz|/archive/refs/tags/v${NEW_VERSION}.tar.gz|" \
		packaging/homebrew/kdal.rb
	rm -f packaging/homebrew/kdal.rb.bak
fi
