---
title: FAQ
description: Frequently asked questions about KDAL.
---
...
hardware. The QEMU aarch64 `virt` machine matches the target ISA (ARM64), so
drivers developed on QEMU require zero code changes on real hardware - only
a backend swap.

### Why not support every device class immediately?

The first milestone validates the **abstraction boundary**, not device
coverage. Starting with four classes (UART, I2C, SPI, GPU) proves the
framework handles both simple serial peripherals and complex accelerators.
...
registry, and events. All hardware logic lives in drivers and backends. This
keeps the trusted surface small and auditable.
...
[Upstreaming](/reference/upstreaming/).
...
route to the selected device's driver. Multiple processes can open `/dev/kdal`
independently, each with their own device selection.

### Why `misc_register` instead of a custom major number?

Automatic minor number allocation avoids conflicts with other drivers and
simplifies deployment.

### How is the ring buffer sized?

The QEMU backend allocates a 4096-byte ring buffer per device, balancing
memory usage with throughput for emulated I/O. Real hardware backends use
DMA buffers or MMIO regions instead.

### Why are KUnit tests separate from integration tests?

KUnit tests run inside the kernel and test internal APIs without userspace.
Integration tests run from userspace and exercise the full ioctl path. Both
are needed: KUnit catches internal regressions, integration tests validate
...
The tool mirrors ioctl ABI definitions locally and does not depend on kernel
headers:
...
states. `.kdc` (KDAL Driver Code) files contain **imperative** event handlers
that implement driver logic. Together they separate *what a device is* from
*how it behaves*.

### How does the KDAL compiler work?

The compiler (`kdalc`) reads a `.kdc` file plus its `.kdh` header, then runs
four pipeline stages:

1. **Lexer** - tokenises source into 80+ token types
...
KDAL reduces a typical driver from 200+ lines of boilerplate C to ~30 lines
of intent. The compiler handles `platform_driver` registration, OF match
table generation, `devm_*` resource management, and error unwinding. New
programmers can focus on driver logic rather than kernel API minutiae.
...
all the C code needed for a loadable kernel module.

### How do I create a custom device header?

Create a `.kdh` file anywhere the compiler can find it (or in
`lang/stdlib/`), following the format in the
[Language Specification](/language/spec/). A minimal `.kdh` file declares a
device name, a register map, and at least one capability. Then reference it
from your `.kdc` with `import "mydevice.kdh";`.
