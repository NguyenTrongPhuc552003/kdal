---
title: Porting Guide
description: How to port KDAL to a new hardware platform by writing a backend adapter.
---

Porting KDAL to a new platform involves implementing a **backend adapter**.
The driver layer and userspace interface remain unchanged - this is the core
value proposition of the abstraction.
...
access MMIO registers, DMA buffers, or a bus controller:

```c
static ssize_t myplatform_read(struct kdal_device *device,
...
implementation. It uses a software ring buffer to emulate hardware I/O:

1. **`init()`** - log initialisation
...
The target hardware platform uses the CIX P1 SoC with:

| Component   | Detail                              |
| ----------- | ----------------------------------- |
| GPU         | Arm Immortalis G720                 |
| NPU         | 30 TOPS neural processing unit      |
| Peripherals | UART, I2C, SPI, GPIO (standard ARM) |
